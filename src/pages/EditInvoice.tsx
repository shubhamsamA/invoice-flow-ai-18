...
export default function EditInvoicePage() {
    ...
    if (invoice && !loaded) {
      ...
      setItems(
        invoice.items.length > 0
          ? invoice.items.map((i: any) => ({ id: i.id, name: i.name, quantity: Number(i.quantity), price: Number(i.unit_price) }))
          : [emptyItem()]
      );
      ...
      if (invoice.layout_json && Array.isArray(invoice.layout_json)) {
        ...
        const match = builtinTemplateOptions.find(
          (t) => t.elements.length === (invoice.layout_json as any[]).length
        );
  ...
  const getSelectedTemplateName = (): string | null => {
    if (!selectedTemplate) return null;
    return builtinTemplateOptions.find((t) => t.id === selectedTemplate)?.name
    ...
    try {
      ...
      const { error: invError } = await supabase
        ...
        .update({
          ...
          subtotal, discount: discountAmount, gst_rate: gstRate, gst_amount: gstAmount, total,
        ...
        <motion.div className="lg:col-span-2 space-y-6" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <div className="bg-card rounded-xl border shadow-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" className="gap-1.5 text-xs" onClick={addItem}><Plus className="h-3.5 w-3.5" /> Add Item</Button>
            </div>
            <div className="space-y-3">
              <div className="grid grid-cols-12 gap-3 text-[10px] uppercase tracking-wider text-muted-foreground font-medium px-1">
                <div className="col-span-5">Description</div><div className="col-span-2">Qty</div><div className="col-span-3">Price</div><div className="col-span-1 text-right">Total</div><div className="col-span-1"></div>
              </div>
              {items.map((item, idx) => (
                <div key={item.id} className="grid grid-cols-12 gap-3 items-center">
                  ...
                  <Button variant="ghost" size="icon" className="col-span-1 h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => removeItem(item.id)} disabled={items.length === 1}><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              ))}
            </div>
          </div>
        ...
        <motion.div className="space-y-4" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2, duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
          <div className="bg-card rounded-xl border shadow-sm p-6 space-y-4 sticky top-6">
            ...
            <div className="space-y-2.5 text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">GST ({gstRate}%)</span><span className="font-medium tabular-nums">+{formatCurrency(gstAmount)}</span></div>
              {discount > 0 && <div className="flex justify-between"><span className="text-muted-foreground">Discount ({discount}%)</span><span className="font-medium text-[hsl(var(--success))] tabular-nums">-{formatCurrency(discountAmount)}</span></div>}
