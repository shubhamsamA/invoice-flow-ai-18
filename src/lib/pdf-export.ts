...
const footerSection = (data: FullInvoiceData) => {
  const parts: string[] = [];
  parts.push(`<div class="footer-text">Generated by ${data.business_name || "InvoiceFlow"} · ${new Date().toLocaleDateString("en-IN")}</div>`);
  ...
  if (data.signature_url) {
    rightParts.push(`<div class="signature-block"><img src="${data.signature_url}" class="signature-img" alt="Signature" /><div class="signature-label">Authorized Signatory</div></div>`);
  }
...
export function exportInvoicePDF(data: SimpleInvoiceData, title = "Invoice") {
  ...
  const itemsRows = data.items.map((item, i) => `
    <tr><td>${i + 1}</td><td>${item.name}</td><td>${item.qty}</td><td>${fmt(item.price)}</td><td style="font-weight:500;">${fmt(item.qty * item.price)}</td></tr>`).join("");
...
export function generateInvoicePreviewHTML(data: FullInvoiceData): string {
  ...
  const statusStyle = data.status === 'paid' ? 'background:#e6f9ee;color:#15803d;' : data.status === 'overdue' ? 'background:#fee2e2;color:#dc2626;' : 'background:#fef3cd;color:#92400e;';
  ...
  const fromHtml = (data.business_name || data.business_email || data.business_address)
    ? `<div>
